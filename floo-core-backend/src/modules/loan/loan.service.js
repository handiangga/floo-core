...
// 🔥 REJECT MANAGER
// ============================
const rejectByManager = async (loan_id, reason) => {
  const loan = await Loan.findByPk(loan_id);
...
// 🔥 GENERATE PDF BUFFER
// ============================
const generateLoanPdf = async (loan_id) => {
  const loan = await Loan.findByPk(loan_id, {
    include: [
      {
        model: Employee,
  ...
  const response = await fetch(loan.loan_agreement);
